...
Contract address is assigned to `-Dcontract` and `-Dmethod` is the method name we are sending the transaction to. `-Dargs` is the keyword for passing in arguments. If your method does not require any arguements, this field is not required. Each [ABI type](#/) has its own [selector](#/) and you need to define it following by the argumentment data. For example, `setString` requires a `string`, where the selector is `-T`.
