...
1. [Native Ubuntu Install](#section-section-native-ubuntu-install)
2. [Docker Image](#section-section-docker-image)
...
The HDD space required only takes the Docker image into account. You will need a significant amount of space for storing the blockchain itself. The database is currently around `22GB` in size, although this can be [pruned](https://github.com/aionnetwork/aion/wiki/Database#section-section-state-database-pruning).
...
Take a look at the [Running the Java Container](#section-section-running-the-java-container) section for more information on what arguments to supply with the `docker run` command.
