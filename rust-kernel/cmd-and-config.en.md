# Command Line and Config File Variables

This page contains the commands available when starting the Rust kernel, as well as their `config.toml` counterparts. Most kernel command line arguments can be set within the `config.toml` file.

The parameter prioritization when starting the kernel is:
