...
Import in a given format. FORMAT must be either 'hex' or 'binary'.
(default: auto)
...
Export in a given format. FORMAT must be either 'hex' or 'binary'.
(default: binary)
...
Export to (including) block BLOCK, which may be an index, hash or
latest.  
*default: "latest"*
...
Revert Database to (including) block BLOCK, which may be an index,
hash.  
*default: "0"*

### db

Manage the database representing the state of the blockchain on this
system
...
Specify the blockchain type. CHAIN may be a JSON chain specification
file.  
*default: "mainnet"*
...
Print DEFAULT configuration to $HOME/.aion/default\_config.toml.

#### no seal check

$ aion --no-seal-check

Skip block seal check. Used to make import and export blocks faster, if
checking seal is not necessary.
...
Specify the cache time of accounts read from disk. If you manage
thousands of accounts set this to 0 to disable refresh.  
*default: 5*
...
Unlock ACCOUNTS for the duration of the execution. ACCOUNTS is a
...
unlock = \[\]
  ...
  - e.g.:unlock
    =\["a05a27f0c1ea16ed4433c9efc86ac08effbf2cd4530d08a2b35393b05e489df5","a064f2fbf5d703733d723ba5f08109d96196467331d5b4568835a634e81c5715"\]
...
Provide a list of files containing passwords for unlocking accounts.
...
sync\_from\_boot\_nodes\_only = false
...
local\_node = "p2p://00000000-0000-0000-0000-000000000000@0.0.0.0:30303"
...
ip\_black\_list = \[\]
  ...
  - e.g.:ip\_black\_list = \["1.2.3.4","2.3.4.5"\]
...
Turn on additional processing threads for JSON-RPC servers (for all
severs i.e for websocket and ipc). Setting this to a non-zero value
allows parallel execution of cpu-heavy queries.

**config:**

\[rpc\]

\#processing\_threads = None

  - e.g.:processing\_threads = 5
...
Specify the hostname portion of the HTTP API server, IP should be an
interface's IP address, or all (all interfaces) or local. all means
0.0.0.0 , local means 127.0.0.1 .  
*default: "local"*
...
Specify the APIs available through the HTTP interface. APIS is a
comma-delimited list of API name. Possible name are all, web3, eth,
stratum, net, personal, rpc. You can also disable a specific API by
putting '-' in the front: all,-personal.NOTE that rpc doesn’t support
pubsub  
**This argument can have multiple .**  
*default: \[ "all", "-pubsub" \]*
...
header sent by the browser, it is additional security against some
...
Specify the APIs available through the WebSockets interface. APIS is a
comma-delimited list of API name. Possible name are web3, eth, stratum,
net, personal, rpc, pubsub.  
**This argument can have multiple .**  
*default: \[ "all", "-pubsub" \]*
...
Specify custom API set available via JSON-RPC over IPC. Possible name
are web3, eth, stratum, net, personal, rpc, pubsub.  
**This argument can have multiple.**  
*default: \[ "all", "-pubsub" \]*
...
Specify the hostname portion of the Wallet API server, IP should be an
interface's IP address, or all (all interfaces) or local. all means
0.0.0.0 , local means 127.0.0.1 .  
*default: "local"*
...
\#zmq\_key\_path = None
  ...
  - e.g.:zmq\_key\_path = "/root/.aion/zmq"
...
Interface address for Stratum server. all means 0.0.0.0 , local means
127.0.0.1 .  
*default: "local"*
...
\#secret = None
  ...
  - e.g.:secret =
    "9b64f3ba5f081016196467331d5b4568835a634e81c575d70352fbf9d9733d72"
...
This gives a slightly faster import speed, but means that extra
solutions submitted for the same work package will go unused.

**config:**

\[mining\]

remove\_solved = false
...
execute all transactions in the queue. Note that such block is invalid
and should never be attempted to be mined.

**config:**

\[mining\]

infinite\_pending\_block = false

#### dynamic gas price

$ aion --dynamic-gas-price

use dynamic gas price which adjust with --gas-price-percentile,
--max-blk-traverse, --blk-price-window

**config:**

\[mining\]

dynamic\_gas\_price = false
...
Specify which transactions should force the node to reseal a block. SET
is one of: none - never reseal on new transactions; own - reseal only on
a new local transaction; ext - reseal only on a new external
...
Specify the minimum time between reseals from incoming transactions. MS
is time measured in milliseconds.  
*default: 4000*
...
Specify the number of historical work packages which are kept cached
lest a solution is found for them later. High values take more memory
but result in fewer unusable solutions.  
*default: 20*
...
Set of transactions to relay. SET may be: cheap - Relay any transacticon
in the queue (this may include invalid transactions); strict - Relay
only executed transactions (this guarantees we don't relay invalid
transactions, but means we relay nothing if not mining); lenient - Same
as strict when mining, and cheap when not.  
*default: "cheap"*
...
A cap on how large we will raise the gas limit per block due to
transaction volume.  
*default: "20000000"*
...
Maximum amount of memory that can be used by the transaction queue.
Setting this parameter to 0 disables limiting.  
*default: 2*
...
tx\_queue\_mem\_limit = 2

#### tx queue strategy

$ aion --tx-queue-strategy=\[S\]

  - e.g.:--tx-queue-strategy=gas\_price

Prioritization strategy used to order transactions in the queue. S may
be: gas - Prioritize txs with low gas limit; gas\_price - Prioritize txs
with high gas price; gas\_factor - Prioritize txs using gas price and
gas limit ratio.  
*default: "gas\_price"*
...
tx\_queue\_strategy = "gas\_price"
...
exceeded before banning sender/recipient/code.  
*default: 1*
...
Banning time (in seconds) for offenders of specified execution time
limit. Also number of offending actions have to reach the threshold
within that time.  
*default: 180*
...
Minimum amount of Wei per GAS to be paid for a transaction to be
accepted for mining.  
*default: 10000000000*
...
Maximum amount of Wei per GAS to be paid for a transaction to be
accepted for mining.  
...
max\_gas\_price = 9000000000000000000
...
Maximum amount of Wei per GAS to be set for a new local transaction to
be accepted for mining when using dynamic gas price.  
...
local\_max\_gas\_price = 100000000000
...
Take BLOCKS blk\_price in blocks which have transactions for dynamic gas
price adjustment. It'll not work without --dynamic-gas-price.  
*default: 20*
...
Maximum amount of blocks can be traversed. It'll not work without
--dynamic-gas-price.  
*default: 64*
...
Set PCT percentile block price value from last blk\_price\_window blocks
as default gas price when sending transactions. It'll not work without
--dynamic-gas-price.  
*default: 60*
...
Specify the block author (aka "coinbase") address for sending block
rewards from sealed blocks. NOTE: MINING WILL NOT WORK WITHOUT THIS
OPTION.
...
\#author = None
  ...
  - e.g.:author =
    "a064f2fbf5d703733d723ba5f08109d96196467331d5b4568835a634e81c5715"
...
Apply a limit of GAS as the maximum amount of gas a single transaction
may have for it to be mined.
...
\#tx\_gas\_limit = None
  ...
  - e.g.:tx\_gas\_limit = "100000000000"
...
senders/recipients/code of transactions offending the limit will be
banned from being included in transaction queue for 180 seconds.

**config:**

\[mining\]

\#tx\_time\_limit = None

  - e.g.:tx\_time\_limit = 600
...
Don't save pending local transactions to disk to be restored whenever
the node restarts.
...
recently synced or default to fast if none synced.  
*default: "archive"*
...
As many states as possible will be kept within this limit, and at least
--pruning-history states will always be kept.  
*default: 32*
...
Database compaction type. TYPE may be one of: ssd - suitable for SSDs
and fast HDDs; hdd - suitable for slow HDDs; auto - determine
automatically.  
*default: "auto"*
...
Amount of verifier threads to use or to begin with, if verifier
...
Specify all modules' log level. LEVEL may be one of: off, error, warn,
info, debug, trace.  
*default: "info"*
...
Specify the log target you want and specify it's log level. Must conform
to the same format as RUST\_LOG.eq 'own\_tx=debug'.  
[Targets Table](https://github.com/aionnetwork/aionr/wiki/Log-Targets)  
**This argument can have multiple.**  
*default: \[\]*
